...
switch (command){
    ...
    function spotifyThisSong(song) {
        ...
        .then(function(response){
            ...
            console.log(error);
            console.log("No Results found. Showing results for 'Fill Me In' by Craig David");
      });
    }

    function errorConditionForSpotify() {
        spotify
        .search({ type: 'track', query: 'Fill Me In' })
        .then(function(response) {
            for (var i=0;i < response.tracks.items.length; i++) {
                if (response.tracks.items[i].artists[0].name === "Craig David") {
        ...
        axios.get(`http://www.omdbapi.com/?t=${movie}&y=&plot=short&apikey=trilogy`).then(
            function(response) {
                ...
                if (response.data.Title != undefined) {
                    ...
                    movieThis("Little Shop of Horrors");
                }
            }
